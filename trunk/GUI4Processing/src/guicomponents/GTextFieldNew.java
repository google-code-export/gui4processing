/*
  ...
  Final modifications made by Brendan Nichols, May 15, 2010 to support multi-line text.
 ...
 *
 */
public class GTextFieldNew extends GComponent {
	...
	private boolean drawSepLines = true; //whether or not to draw separating lines between each line, really only needed in my application
	 ...
	 */
	public GTextFieldNew(PApplet theApplet, String text, int x, int y, int width, int height){
	 ...
	 * @param height height of text field
	 */
	private void textFieldCtorCore(String text, int width, int height) { //don't know what ctors is but I'll leave as is
		...
		border = 1; //set the border of text box as default 1, added functions to change and read this value
		leading = (int)1.5 * localFont.getFont().getSize(); //default line leading is 1.5 times the font height
		...
		createEventHandler(winApp, "handleTextFieldEvents", new Class[]{ GTextFieldNew.class }); //create the event handler for user to code
		registerAutos_DMPK(true, true, false, true); //register something, no clue what though
		winApp.textFont(localFont, localFont.getFont().getSize()); //set the text font
	}
	...
	private Point cursorPos2D(int x, int y){
		...
		String[] splittext = text.split("\n"); //split the text into lines
		int cursorY = (int) y / leading + startY; //take nearest integer multiple of line height and add to starting line number
	...
	private String viewText() {
		...
		for(int i = startY; i < endY; i++){ //take from startY line to endY line
			if(i < splittext.length){
				int charWidth; //initialize for the loop, which will determine how many characters can fit on one line
				for(charWidth = startX; charWidth < splittext[i].length(); charWidth++){ //loop from start to end of line
					if(winApp.textWidth(splittext[i].substring(startX, charWidth)) > (width - 8)){ //stopping when the width exceeds the box width
						charWidth --; //subtract one since we went over the width by one character
						break;
					}
				}
				//take the viewable text on the current line
				cuttext[i-startY] = splittext[i].substring(Math.min(startX, splittext[i].length()), Math.min(charWidth, splittext[i].length()));
			}
			else {
				cuttext[i-startY] = "";
			}
		}
		String jointext = winApp.join(cuttext, "\n"); //join lines together again with line breaks
		return jointext;
	}
	...
	private void showCursor(){
		if(cursorPosition(cursorPos).x < startX){ //cursor went out to the left
			while (cursorPosition(cursorPos).x < startX){ //move the text until inside
				startX --;
			}
		}
		if(cursorPixPosition(cursorPos).x > width - 8){ //cursor went out to the right
			while (cursorPixPosition(cursorPos).x > width - 8){ //move the text until inside
				startX ++;
			}
		}
		if(cursorPosition(cursorPos).y < startY){ //cursor went out at the top
			while (cursorPosition(cursorPos).y < startY){ //move the text until inside
				startY --;
				endY --;
			}
		}
		if(cursorPosition(cursorPos).y > endY - 2){ //cursor went out past the bottom
			while (cursorPosition(cursorPos).y > endY - 2){ //move the text until inside
				startY ++;
				endY ++;
			}
		}
	...
	public void draw () {
		...
		if(border > 0){winApp.strokeWeight(border);}else{winApp.noStroke();}
		...
		if(startSelect != endSelect){ //if something is selected
			...
			for(int i = Math.min(startSelect, endSelect); i < Math.max(startSelect, endSelect); i++){
				if(cursorPosition(i).x >= startX && cursorPixPosition(i).x < (width - 8) &&
					...
					winApp.rect(pos.x + 4 + cursorPixPosition(i).x, pos.y + 4 + cursorPixPosition(i).y,
							Math.min(winApp.textWidth(text.substring(i, i+1)), width - 8 - cursorPixPosition(i).x),localFont.getFont().getSize());
				}
			}
		}
		...
		winApp.text (viewText(), pos.x + 4, pos.y + 4, width - 8, height - 8);
		...
		if(focusIsWith == this && (winApp.millis() % 1000) > 500) {
			...
			winApp.line(pos.x + 4 + cursorPix.x, pos.y + cursorPix.y + 4, pos.x + 4 + cursorPix.x, pos.y + cursorPix.y + 4 + localFont.getFont().getSize());
			winApp.fill(localColor.txfFont);
		}
